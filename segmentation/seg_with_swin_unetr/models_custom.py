...
class GlobalAttentionBlock(nn.Module):
    """
    A global self-attention block that auto-initializes to the input embedding dim.
    """
    def __init__(self, num_heads=8, qkv_bias=True, attn_drop=0.0, proj_drop=0.0):
        super().__init__()
        self.num_heads = num_heads
        self.qkv_bias = qkv_bias
        self.attn_drop_rate = attn_drop
        self.proj_drop_rate = proj_drop
        self.qkv = None  # Lazy init
        self.proj = None
        self.scale = None
        self.softmax = nn.Softmax(dim=-1)

    def build_layers(self, C, device):
        if C % self.num_heads != 0:
            raise ValueError(f"Embedding dim {C} not divisible by num_heads {self.num_heads}.")
        head_dim = C // self.num_heads
        self.scale = head_dim ** -0.5
        self.qkv = nn.Linear(C, C * 3, bias=self.qkv_bias).to(device)
        self.proj = nn.Linear(C, C).to(device)
    ...
    def forward(self, x):
        ...
        B = x.shape[0]
        if len(orig_shape) == 5:
            x = x.reshape(B, -1, orig_shape[-1])  # (B, N, C)
        elif len(orig_shape) == 4:
            x = x.reshape(B, -1, orig_shape[-1])
        else:
        ...
        # reshape lại
        if len(orig_shape) == 5:
            out = out.reshape(B, orig_shape[1], orig_shape[2], orig_shape[3], C)
        elif len(orig_shape) == 4:
            out = out.reshape(B, orig_shape[1], orig_shape[2], C)
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, normalize=True):
        ...
        if self.use_global_attn:
            x4_in = None
            if len(x4_out.shape)==5:
                x4_in = x4_out.permute(0,2,3,4,1).contiguous()
            elif len(x4_out.shape)==4:
                x4_in = x4_out.permute(0,2,3,1).contiguous()
            x4_global = self.global_attn(x4_in)
            if len(x4_out.shape)==5:
                x4_global = x4_global.permute(0,4,1,2,3).contiguous()
            elif len(x4_out.shape)==4:
                x4_global = x4_global.permute(0,3,1,2).contiguous()
            x4_out = x4_out + x4_global
